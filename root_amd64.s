...
// runSkip32 returns the initial byte class run, rather than the prefix before
// a root candidate. Each equality bit is inverted so its first set bit is the
// first nonmatching byte.
TEXT ·runSkip32(SB), NOSPLIT, $0-40
	MOVQ ptr+0(FP), AX
	MOVQ n+8(FP), DX
	XORQ BX, BX
	MOVQ target+16(FP), CX
...
// runSkip64 is the AVX-512 BW counterpart of runSkip32.
TEXT ·runSkip64(SB), NOSPLIT, $0-40
	MOVQ ptr+0(FP), AX
	MOVQ n+8(FP), DX
	XORQ BX, BX
	MOVQ $-1, CX
