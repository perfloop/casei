...
case-insensitive literal benchmark `sherlock-casei-en`:
...
dedicated caseless substring engine exists in that suite at all. The caseless
column is contested only by general regex engines.
...
5. **Estimation is not measurement**: predicted speedups in this domain are
